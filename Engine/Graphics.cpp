...
void Graphics::DrawSpriteNonChroma(int x, int y, RectI srcRect, const RectI& clip, const Surface& s)
{
	...
	for (int sy = srcRect.top; sy < srcRect.bottom; sy++)
	{
		...
		{
			// Subtrack the corner of the rectangle to fix the offset between the image and the screen
	...
	for (int sy = srcRect.top; sy < srcRect.bottom; sy++)
	{
		for (int sx = srcRect.left; sx < srcRect.right; sx++)
		{
			...
			if (srcPixel != chroma)
			{
				...
				// Blend color between the color of the source and the color of the screen
				const int xDest = x + sx - srcRect.left;
				const int yDest = y + sy - srcRect.top;
				const Color dstPixel = GetPixel(xDest, yDest);

				// Blending color 50% each color
				const Color blendPixel = Color
					{
				...
				PutPixel(xDest, yDest, blendPixel);
			}
		}
	}
